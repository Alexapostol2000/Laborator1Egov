...
const EndDashboard = ({room, token}) => {
    ...
    return (
        ...
        <div className="card shadow mb-1 mx-auto text-center" style ={{justifyContent:'center'}} />
                  <h5 className="card-title"> Camera: {room} </h5>
                  <h6 className="card-subtitle mb-2 text-muted"> Numar utilizatori: {usersScore.length} </h6>
                {usersScore.map((user) => {
                    return (
                        <div className="row">
                            <div className="col md-6">
                                <div className="fw-bold" style={{ color: '#598392' }}> Playeri </div>
                                <ol className="list-group ">
                                    <li className="list-group-item d-flex justify-content-between align-items-start" key={user.id} style={{ backgroundColor: '#85BAA1' }}>
                                        
                                        <div className="fw-bold" style={{ color: '#598392' }}> {user.userName}</div>
                                    </li>
                                </ol>
                            </div>
                            <div className="col md-6">
                                <div className="fw-bold" style={{ color: '#598392' }}> Score </div>
                                <ol className="list-group ">
                                    <li className="list-group-item d-flex justify-content-between align-items-start" key={user.id} style={{ backgroundColor: '#85BAA1' }}>
                                    
                                        <div className="fw-bold" style={{ color: '#598392' }}> {user.score}</div>
                                    </li>
                                </ol>
                            </div>
                        </div>
